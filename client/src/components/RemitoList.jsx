...
const RemitoList = () => {
    ...
    const handleDelete = async (e, remito) => {
        e.stopPropagation(); // Prevent row click

        if (!window.confirm('¿Estás seguro de que quieres eliminar este conteo? Esta acción no se puede deshacer y borrará permanentemente el registro.')) {
            return;
        }
        ...
        try {
            ...
            if (remito.type === 'remito') {
                endpoint = `/api/remitos/${remito.id}`;
            } else if (remito.type === 'pre_remito') {
                endpoint = `/api/pre-remitos/${remito.id}`;
            } else if (remito.type === 'general_count') {
                endpoint = `/api/general-counts/${remito.id}`;
            } else {
                // Fallback or error if type missing (legacy support?)
                // If is_finalized, assume remito. If not, check other props.
                // But better to rely on type.
                if (remito.is_finalized) endpoint = `/api/remitos/${remito.id}`;
                else if (remito.items && remito.items.length > 0) endpoint = `/api/pre-remitos/${remito.id}`;
                else endpoint = `/api/general-counts/${remito.id}`;
            }

            if (!endpoint) throw new Error("No se pudo determinar el tipo de conteo");
            ...
            alert('Error al eliminar el conteo: ' + (error.response?.data?.message || error.message));
        }
    };
