...
// Get all remitos with manual join to pre-remitos/PV, and include Pending pre-remitos (Progress) - Optimized via Supabase RPC
router.get('/remitos', verifyToken, async (req, res) => {
    try {
        const { data, error } = await supabase
            .rpc('get_remitos_consolidado', {
                p_user_role: req.user.role,
                p_user_sucursal_id: req.user.sucursal_id || null
            });

        if (error) throw error;
        res.json(data || []);
    } catch (error) {
        console.error('Error fetching remitos via RPC:', error);
        res.status(500).json({ message: 'Internal server error' });
    }
});
...
router.get('/general-counts/:id/product-list', verifyToken, async (req, res) => {
    ...
    try {
        ...
        // 5. Apply Global Sorting (Logical Order: Capacity -> Brand -> Excel Order -> Description)
