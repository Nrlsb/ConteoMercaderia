...
CREATE TABLE IF NOT EXISTS dye_counting_lists (
    ...
    sucursal_id UUID, -- Cambiado de INTEGER a UUID
    created_by TEXT,
    deleted_at TIMESTAMPTZ
);

CREATE TABLE IF NOT EXISTS dye_count_items ( -- Renombrado de dye_counting_items para coincidir con el código
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
ALTER TABLE dye_count_items ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Allow all for authenticated" ON dye_count_items FOR ALL USING (true);
